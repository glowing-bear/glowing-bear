 ...
 *
 * URL plugins are created by providing a name, an optional pattern and a
 * function to call with the URL and optional matches which returns any
 * additional content.
 */
    ...
    return {
        contentForMessage: function(message) {
            ...
            for (var i = 0; urls && i < urls.length; i++) {
                ...
                // Call plugin function only if no pattern or pattern matches
                // Note that match(/null/i) returns null
                if (!urlPattern || match) {
                    var result = urlCallback(url, match);
                    if (result) {
                        content.push(result);
                    }
                }
...
plugins.factory('userPlugins', function() {
     ...
     */
    var youtubePattern = '(?:youtube.com|youtu.be)/(?:v/|embed/|watch(?:\\?v=|/))?([a-zA-Z0-9-]+)';
    var youtubePlugin = new UrlPlugin('YouTube video', youtubePattern, function(url, match) {
        var token = match[1];
     ...
     * Image Preview
     */
    var imagePattern = '\\.(png|gif|jpg|jpeg)(:(small|medium|large))?\\b';
    var imagePlugin = new UrlPlugin('image', imagePattern, function(url) {
     ...
     */
    var cloudmusicPlugin = new UrlPlugin('cloud music', null, function(url) {
     ...
     * Google Maps
     */
    var googlemapPlugin = new UrlPlugin('Google Map', null, function(url) {
    ...
    /*
      * Asciinema plugin
     */
    var asciinemaPattern = '\\basciinema.org/a/(\\d+)';
    var asciinemaPlugin = new UrlPlugin('ascii cast', asciinemaPattern, function(url, match) {
        var id = match[1];
        return function() {
    ...
    var yrPattern = '\\byr.no/(place|stad|sted|sadji|paikka)/(([^\\s.;,(){}<>/]+/){3,})';
    var yrPlugin = new UrlPlugin('meteogram', yrPattern, function(url, match) {
    ...
    // Embed GitHub gists
    var gistPattern = '\\bgist\\.github.com/[^.?]+';
    var gistPlugin = new UrlPlugin('Gist', gistPattern, function(url, match) {
    ...
    var tweetPattern = '\\btwitter\\.com/(?:#!/)?(\\w+)/status(?:es)?/(\\d+)';
    var tweetPlugin = new UrlPlugin('Tweet', tweetPattern, function(url, match) {
     ...
     * Vine plugin
     */
    var vinePattern = '\\bvine.co/v/([a-zA-Z0-9]+)(/.*)?';
    var vinePlugin = new UrlPlugin('Vine', vinePattern, function (url, match) {
        var id = match[2];
